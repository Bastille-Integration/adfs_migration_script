...
`-TargetAdfsHostname` is required in non-interactive mode **unless** `-Site` is used — with `-Site` it defaults to the current federation host. Without `-Site`, the script exits with an error if it is omitted non-interactively.
...
- **Every run cleans up old configuration — not only with `-Site`.** Redirects and CORS are full/clean replaces (above); stale **HTTP.SYS SSL bindings for the old certificate** are swept; and **leftover federation-host bindings** (the previous host *and* any older sibling hosts such as `auth.adfs.<d>` / `auth-office.adfs.<d>` / `auth-backup.adfs.<d>`) are removed on every SSL-updating run. The federation-host sweep is scoped to the ADFS host's own parent domain, so app hosts and unrelated `:443` sites are never touched. (Use `-SkipAdfsSsl` to skip binding cleanup, `-SkipCors` / `-SkipFederationServiceProperties` to skip those.)
